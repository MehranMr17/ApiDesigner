...
import { defaultProject, errorDefaultSchema, loadProject, makeField, saveProject } from '../lib/project';
import type { ApiEdge, ApiNode, ApiNodeType, SchemaField } from '../types';
...
export const useApiDesignerStore = create<State>((set, get) => ({
  nodes: initial.nodes,
  edges: initial.edges,
  ...
  onConnect: (connection) => set((s) => ({ edges: addEdge({ ...connection, id: id(), data: { label: 'Flow' }, label: 'Flow' }, s.edges) as ApiEdge[] })),
  updateNode: (nodeId, patch) => set((s) => ({ nodes: s.nodes.map((n) => (n.id === nodeId ? { ...n, data: { ...n.data, ...patch } } : n)) })),
  addNode: (type) => set((s) => {
    const nodeId = id();
    ...
    const base = {
      ...
      data: {
        ...
        schema: type === 'error' ? errorDefaultSchema() : [makeField('field', 'string')],
      },
    } as ApiNode;
    const edges = [...s.edges];
  ...
  deleteSelected: () => set((s) => {
    ...
    return { nodes: s.nodes.filter((n) => n.id !== s.selectedNodeId), edges: s.edges.filter((e) => e.source !== s.selectedNodeId && e.target !== s.selectedNodeId), selectedNodeId: null };
  }),
  addField: (nodeId) => set((s) => ({ nodes: s.nodes.map((n) => (n.id === nodeId ? { ...n, data: { ...n.data, schema: [...n.data.schema, makeField('field', 'string', false)] } } : n)) })),
  updateField: (nodeId, fieldId, patch) => set((s) => ({ nodes: s.nodes.map((n) => n.id === nodeId ? { ...n, data: { ...n.data, schema: n.data.schema.map((f) => f.id === fieldId ? { ...f, ...patch } : f) } } : n) })),
  removeField: (nodeId, fieldId) => set((s) => ({ nodes: s.nodes.map((n) => n.id === nodeId ? { ...n, data: { ...n.data, schema: n.data.schema.filter((f) => f.id !== fieldId) } } : n) })),
  toggleStatus: (status) => set((s) => ({ enabledStatuses: s.enabledStatuses.includes(status) ? s.enabledStatuses.filter((x) => x !== status) : [...s.enabledStatuses, status].sort((a, b) => a - b) })),
  autoLayout: () => set((s) => ({ nodes: s.nodes.map((n, i) => ({ ...n, position: { x: 120 + (i % 3) * 340, y: 100 + Math.floor(i / 3) * 220 } })) })),
  saveLocal: () => {
    const { nodes, edges, selectedNodeId } = get();
    saveProject({ nodes, edges, selectedNodeId });
  },
  resetDefault: () => {
